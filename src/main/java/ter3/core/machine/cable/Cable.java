...
public class Cable extends DefBlock {

    String name;

    public Cable(Material m, SoundType s, String n) {

        super(DefBlock.build(1.5, 2, m, s, ToolType.PICKAXE, 2, 0, false));
        name = n;
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return TileInit.getType(name).create();
    }
    ...
    private int connectType(@Nonnull IWorld world, @Nonnull Direction facing, BlockPos pos) {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if(handIn == Hand.MAIN_HAND && !worldIn.isRemote()) {
            if(MachinePostEvent.clickMachineEvent(worldIn, pos, player, hit)) {
                /*
                if(!player.isSneaking() && player.getHeldItemMainhand().getRcp() instanceof BlockItem) {
                    Block block = ((BlockItem) player.getHeldItemMainhand().getRcp()).getBlock();
                    TileEntity t = worldIn.getTileEntity(pos);
                    if(t != null && !(block instanceof Cable) && block.getDefaultState().isSolid()) {
                        ((CableTile) t).put(block);
                    }
    ...
    public void neighborChanged(BlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos, boolean isMoving) {
        ...
        if(!worldIn.isRemote()) {
            ...
            if(t instanceof CableTile) {
                EnergyNetManager.getRcp(worldIn).enableBlock(pos, t::markDirty);
            }
        }
